...
export interface LevelTask {
  ...
  text: string;
  hint: string;
  check: (git: Git, ctx: { history: string[] }) => boolean;
}
...
function ran(ctx: { history: string[] }, re: RegExp): boolean {
  return ctx.history.some((c) => re.test(c.trim()));
}
function commits(git: Git): number {
  return Object.keys(git.repo.commits).length;
}
function branchNames(git: Git): string[] {
  return Object.keys(git.repo.branches);
}
function featureBranches(git: Git): string[] {
  return branchNames(git).filter((b) => b !== 'main');
}
function anyCommitMsg(git: Git, re: RegExp): boolean {
  const c = git.repo.commits;
  return Object.keys(c).some((s) => re.test(c[s].msg));
}

export const LEVELS_DATA: Record<number, LevelDefinition> = {
  1: {
    id: 1,
    ...
    tasks: [
      ...
      },
      {
        ...
        check: (g) => g.repo.index['README.md'] !== undefined,
      },
      {
        id: 'commit_readme',
        short: '5. first commit frozen',
  ...
  2: {
    ...
    intro: [
      'A repo with two commits on main, and a branch called quick-fix that somebody else already finished.',
      'Your job: do your own work on a branch, merge it, then deal with the conflict that quick-fix has been waiting to cause.',
    ],
    ...
    tasks: [
      ...
      },
      {
        ...
        check: (g) => branchNames(g).length >= 3 && g.currentBranch() !== 'main',
      },
      {
        id: 'work',
        short: 'committed on the branch',
        ...
        check: (g) => {
          const b = g.currentBranch();
          if (!b || b === 'main') {
            return featureBranches(g).some(
              (x) => x !== 'quick-fix' && g.repo.branches[x] !== g.repo.branches.main && commits(g) >= 4
            );
          }
      ...
      },
      {
        ...
        check: (g, ctx) => ran(ctx, /^git (checkout|switch) main$/) && g.currentBranch() === 'main',
      },
      {
        id: 'ff',
        short: 'fast-forward merge done',
        ...
        check: (g, ctx) => {
          ...
          return (
            featureBranches(g).some((b) => b !== 'quick-fix' && g.isAncestor(g.repo.branches[b], g.repo.branches.main)) ||
      ...
      },
      {
        ...
        check: (g, ctx) => ran(ctx, /^git branch -d/) && branchNames(g).length <= 2,
      },
      {
        id: 'conflict',
        short: 'conflict triggered',
        ...
        check: (g, ctx) => ran(ctx, /^git merge quick-fix/),
      },
      {
        id: 'resolve',
        short: 'conflict resolved',
        ...
        check: (g) => g.repo.conflict !== null && g.repo.conflict.files.length === 0,
      },
      {
        id: 'seal',
        short: 'merge commit created',
        ...
        check: (g) => {
          const c = g.repo.commits;
          return Object.keys(c).some((s) => c[s].parents.length > 1);
        },
      },
    ],
  },
  3: {
    ...
    tasks: [
      ...
      },
      {
        ...
        check: (g) => !!g.repo.claimed,
      },
      {
        id: 'fetch',
        short: 'upstream fetched',
        ...
        check: (g) => g.repo.remoteRefs['upstream/main'] === g.repo.servers.upstream.branches.main,
      },
      {
        id: 'sync',
        short: 'main synced',
        ...
        check: (g) => g.repo.branches.main === g.repo.servers.upstream.branches.main,
      },
      {
        id: 'pushmain',
        short: 'fork updated',
        ...
        check: (g) => g.repo.servers.origin.branches.main === g.repo.branches.main,
      },
      {
        id: 'branch',
        short: 'work branch created',
        ...
        check: (g) => featureBranches(g).length >= 1,
      },
      {
        id: 'commit',
        short: 'work committed',
        ...
        check: (g) => featureBranches(g).some((b) => g.repo.branches[b] !== g.repo.branches.main),
      },
      {
        id: 'pushbranch',
        short: 'branch pushed to fork',
        ...
        check: (g) => {
          const srv = g.repo.servers.origin.branches;
          return Object.keys(srv).some((b) => b !== 'main');
        },
      },
      {
        id: 'pr',
        short: 'pull request opened',
      ...
      },
      {
        ...
        check: (g, ctx) => !!g.repo.pr && g.repo.pr.state === 'MERGED' && ran(ctx, /^git log/),
      },
      {
        id: 'resync',
        short: 'fork re-synced',
      ...
      },
      {
        ...
        check: (g) => {
          const localGone = featureBranches(g).length === 0;
          const remoteGone = Object.keys(g.repo.servers.origin.branches).every((b) => b === 'main');
  ...
  4: {
    ...
    intro: [
      ...
      'This level is about the commands people are scared of, in the order that makes them stop being scary.',
    ],
    ...
    tasks: [
      {
        ...
        check: (g, ctx) => ran(ctx, /^git stash\s*$/) || ran(ctx, /^git stash push/),
      },
      {
        id: 'pop',
        short: 'work in progress restored',
        text: 'Bring it back with <code>git stash pop</code>. Stash is a shelf, not a bin.',
        hint: 'git stash pop',
        check: (g, ctx) => ran(ctx, /^git stash (pop|apply)/),
      },
      {
        id: 'rebase',
        short: 'branch rebased',
      ...
      {
        id: 'lease',
        short: 'rebased branch pushed safely',
      ...
      },
      {
        ...
        check: (g, ctx) => ran(ctx, /^git commit --amend/),
      },
      {
        id: 'reset',
        short: 'commit thrown away',
        ...
        check: (g, ctx) => ran(ctx, /^git reset --hard/),
      },
      {
        id: 'rescue',
        short: 'lost commit recovered',
        ...
        check: (g, ctx) => ran(ctx, /^git reflog/) && branchNames(g).some((b) => /^rescue/.test(b)),
      },
      {
        id: 'cherry',
        short: 'commit cherry-picked',
        ...
        check: (g, ctx) => ran(ctx, /^git cherry-pick/) && !ran(ctx, /^git cherry-pick\s*$/),
      },
      {
        id: 'revert',
        short: 'change safely undone',
